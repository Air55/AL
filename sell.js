...
const RETRY_COUNT = 2;
const listforever = true;
const listTime = 1440; //m
const intervalTime = 4000; //12000 //18000; //ms
...
async function main() {
  ...
  try {
    ...
    if (listforever) expirationTime = Math.round(current_time + 3 * 30 * 60 * listTime);
    ...
    const fixedPriceSellOrder = await openseaSDK.createSellOrderLegacyWyvern({
      asset: {
        tokenId:tokenId,
        tokenAddress:NFT_CONTRACT_ADDRESS,
        schemaName: WyvernSchemaName.ERC1155
    ...
    Logger.success(`Successfully created a order! ${fixedPriceSellOrder.asset.openseaLink}, tokenId: ${tokenId}, cost sec = ${(Date.now()/1000 - current_time).toFixed(2)}, current_index: ${current_index}` );
    ...
    Logger.err(`logerr: ${e}, err_retrycount: ${err_retrycount}, current_index: ${current_index}`);
    err_retrycount += 1;
...
async function start() {
  ...
  await wait(2000); //5000 //ms
  main();
}

start().catch(err => {
  Logger.err(`Start err: ${err}`); //858
});